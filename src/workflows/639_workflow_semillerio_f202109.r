...
HT_tuning_base <- function( pinputexps, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    min_data_in_leaf = c( 80, 10000L, "integer" ),
...
wf_sept_semillerio <- function( pnombrewf )
{
  ...
  CA_catastrophe_base( metodo="Ninguno") # Cambio
  FEintra_base() # Cambio
  DR_drifting_base(metodo="uva") # Cambio
  FEhist_base()
  FErf_attributes_base()
  #CN_canaritos_asesinos_base(ratio=0.2, desvio=4.0)

  ts9 <- TS_strategy_base9() # Cambio
  ht <- HT_tuning_base() # Cambio
